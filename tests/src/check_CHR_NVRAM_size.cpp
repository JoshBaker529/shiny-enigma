...
int check_CHR_NVRAM_size(int argc, char *argv[]) {

  std::ifstream inFile;
  Header header;
  int value;

  inFile.open("../../inputs/chr_nvram_size_0.nes_test");
  if (!inFile) {
  ...
  header.read(inFile);
  inFile.close();
  value = header.get_CHR_NVRAM_size();
  if (value != 0) {
    std::cout << "Function returned " << value << ", should have returned 0";
    return 1;
  }

  inFile.open("../../inputs/chr_nvram_size_F.nes_test");
  if (!inFile) {
